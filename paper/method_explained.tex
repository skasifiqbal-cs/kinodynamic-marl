...
This step exists only because stops are expensive. Under the smooth construction of
\S\ref{sec:flat} the waypoints are not events at all --- nothing stops at them --- so there
is nothing to delete, and the simplification is skipped entirely: the curve is fitted to
the full dense route instead.
...
\item Once the device is chosen, trajectories follow in closed form, with no solver. Either
construction will do it: stop-and-go from trapezoidal profiles, or --- because the robot is
differentially flat --- a single smooth curve whose timing is fixed by a speed profile along
it, which is both faster and able to use both controls at once.
