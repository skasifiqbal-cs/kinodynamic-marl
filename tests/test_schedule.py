...
`forbidden` is the load-bearing piece: it decides, for one pair of traversals, exactly
...
class _Env:
    """Just the two attributes `forbidden` reads."""
...
def test_two_robots_on_one_track_collide_only_at_small_delay_differences():
    ...
    clamping `forbidden` models. So the far tail must stay forbidden too, and the answer
    is one contiguous band rather than a small window near zero.
    """
    env, track = _Env(0.25), _walk(0.0, 0.0, 100)
    bad = forbidden(env, 0, 1, track, track.copy(), step=5, kmax=40, clearance=0.05)
    assert 0 in bad
    assert runs(bad) == [(min(bad), max(bad))], "expected one contiguous band"
...
def test_tracks_a_body_apart_never_collide_at_any_delay():
    ...
    assert forbidden(env, 0, 1, a, b, step=5, kmax=40, clearance=0.05) == []
...
def test_a_crossing_pair_is_free_once_one_is_let_through():
    ...
    b = np.column_stack([np.zeros(100), -2.5 + 0.05 * np.arange(100),
                         np.full(100, np.pi / 2)])
    bad = forbidden(env, 0, 1, a, b, step=5, kmax=60, clearance=0.05)
