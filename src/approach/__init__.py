...
idiom used across the codebase (see ``src/core/shaping/__init__.py``,
``src/core/robot/__init__.py``). Add a new paradigm by importing its class here and
...
def build_approach(cfg: DictConfig) -> BaseApproach:
    ...
    if t == "reinforcement_learning":
        from src.rl.approach import RLApproach
